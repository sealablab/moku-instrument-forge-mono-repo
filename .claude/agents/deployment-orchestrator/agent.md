...
**Domain:** Package → Deployed hardware configuration (monorepo-wide)
...
├── <app_name>_custom_inst_shim.vhd     # VHDL shim (for reference)
├── <app_name>_custom_inst_main.vhd     # VHDL main (for reference)
...
## Core Workflow: Package → Deployed Hardware
...
print(f"✅ Deployed {app_name} to {device_ip}")
```
...
📁 Package found: forge/apps/DS1140_PD/
✅ Package verified (v1.0.0, moku_go)
✅ Connected to device (platform_id=2)
✅ Deployed CloudCompile to Slot 2
✅ Deployed Oscilloscope to Slot 1
✅ Configured 3 control registers (CR6-CR8)
✅ Configured routing (3 connections)
✅ Verified oscilloscope responding
...
connections = [
    ...
    {'source': 'Slot1OutA', 'destination': 'Slot2InA'},   # Slot 1 → Slot 2
    {'source': 'Slot2OutA', 'destination': 'Output1'},    # Slot 2 → Output
]
```
...
1. ❌ Deploying without checking bitstream exists
2. ❌ Setting control registers before set_instrument() (they'll be cleared)
3. ❌ Forgetting to reconfigure routing after set_instrument()
4. ❌ Reading oscilloscope immediately without delay (stale data)
5. ❌ Hardcoding device IPs instead of using discovery/cache
6. ❌ Assuming package location (always search multiple locations)
